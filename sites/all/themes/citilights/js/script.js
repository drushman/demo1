...
! function($) {
  $(document).ready(function() {
    ...
    (function() {
      if($('.lot-area-slider-range').length){
        $(".lot-area-slider-range").noUiSlider({
          start: [parseInt(Drupal.settings.citilights.setting.lot_area_min), parseInt(Drupal.settings.citilights.setting.lot_area_max)],
          range: {
            'min': parseInt(Drupal.settings.citilights.setting.config_lot_area_min),
            'max': parseInt(Drupal.settings.citilights.setting.config_lot_area_max)
          },
          step: 1,
          format: wNumb({
            decimals: 0,
        ...
        $(".lot-area-slider-range").Link('lower').to('-inline-<div class="tooltip"></div>', function ( value ) {
          $(this).html(
            '<span>' + value + '</span>'
          );
        });

        $(".lot-area-slider-range").Link('upper').to('-inline-<div class="tooltip"></div>', function ( value ) {
          $(this).html(
            '<span>' + value + '</span>'
          );
        });
